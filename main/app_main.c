...
static const char *pbox_status_label(pbox_status_t status) {
    switch (status) {
        case PBOX_STATUS_READY:
            return "ready";
        case PBOX_STATUS_ARMED:
            return "armed";
        case PBOX_STATUS_RUNNING:
            return "running";
        case PBOX_STATUS_FINISHED:
            return "finished";
        default:
            return "unknown";
    }
...
static void apply_settings_action(void) {
    switch (s_ctx.settings_selected) {
        ...
        case SETTINGS_OPTION_GNSS_DYNAMIC: {
            gnss_dynamic_mode_t next = next_dynamic_mode(s_ctx.gnss_dynamic_mode);
...
static void sensor_task(void *arg) {
    ...
    while (1) {
        ...
        const char *context = (s_ctx.mode == MODE_PBOX) ? pbox_status_label(s_ctx.pbox_status) : "ride";
