...
- 9 项配置条目及其动作规范：
  ...
  9. **SENSOR CALIB**：短按启动传感器分步校准状态机：
     - a. **IMU 校准前 5 秒准备倒计时**：界面大字显示 5s 倒计时，提示“请将设备水平平放好”；
     - b. **IMU 零偏校准（3 秒）**：设备保持水平静止，采集三轴陀螺零漂与重力基准；
     - c. **地磁校准前 5 秒准备倒计时**：界面大字显示 5s 倒计时，提示“请拿起设备准备在空中做 8 字晃动”；
     - d. **地磁空中 8 字校准（6 秒）**：用户在空间中做“8 字晃动”，拟合软硬铁三维椭球；**约束：未完成 8 字校准前严禁结束**；
     - e. **完成判定**：只有在 8 字校准完全达标收敛后，状态机才置位 `CALIB_PHASE_DONE`，界面居中呈现显著的 **“校准 OK”**（`[OK] CALIBRATION SUCCESS`）字样与 **“保存退出”**（`SHORT: SAVE & EXIT`）提示；此时短按按键写入 NVS `cal` 分区并安全退出；
