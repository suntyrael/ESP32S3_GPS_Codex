...
- **真实传感器校准算法与数据持久化机制**：
  ...
  - **IMU 真实零偏对准**：实时监测三轴角速度与加速度合力模长，**只有处于真实水平静止状态时才累加有效采样**；若检测到设备在晃动，进度条绝对停滞并红字告警；
  - **地磁真实 3D 八象限空间拟合**：实时追踪三维磁场极值跨度 $(\Delta X, \Delta Y, \Delta Z)$ 与三维空间八卦象立体角覆盖率；**若设备静止放在桌上，由于无空间旋转，进度条绝对恒为 0%**！只有当用户在空中真实翻转做 8 字晃动、覆盖至少 7 个空间象限且动态范围充足时，进度条才涨至 100% 并收敛；
