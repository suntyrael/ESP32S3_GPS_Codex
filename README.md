...
- **校准机制与安全取消**：
  ...
  - **阶段 1：IMU 校准前 5 秒倒计时准备期**，大字显示 `COUNTDOWN: 5 S`，提示用户将设备水平静止放置好；
  - **阶段 2：IMU 水平静止采样（3 秒）**，界面显著提示 `>> KEEP FLAT & STILL! <<` 并显示采样进度；
  - **阶段 3：地磁校准前 5 秒倒计时准备期**，大字显示 `COUNTDOWN: 5 S`，提示用户拿起设备准备做 8 字晃动；
  - **阶段 4：地磁计空中 8 字晃动采样（6 秒）**，界面明确提示 `>> ROTATE IN FIGURE-8! <<`，**未完成 8 字校准前严禁提前结束（短按不生效）**；
  - **阶段 5：校准完成**：只有在 8 字校准完全达标收敛后，界面才进入完成态，显著呈现 **“校准 OK”**（`[OK] CALIBRATION SUCCESS`）字样与 **“保存退出”**（`SHORT: SAVE & EXIT`）提示，此时短按按键保存矩阵落盘 NVS 并安全退出；
  - 若用户在任意阶段需要放弃，**长按波轮（≥800ms）**随时直接中止并安全回退至设置列表上级；
