...
| 操作 | 场景与效果 |
| --- | --- |
| 旋转编码器 | 顺序切换 3 个大页面（Page 0 ↔ Page 1 ↔ Page 2） |
| 短按（SHORT） | - **P-Box 模式**：清零复位（RESET），重新进入就绪待机；<br>- **轨迹记录**：开启轨迹记录（防误触：开启短按）；<br>- **码表模式**：暂停 / 继续单次骑行；<br>- **设置页**：循环步进修改当前选中项的值并即时生效（Page 0 实时联动）。 |
| 长按（LONG，>900ms） | - **轨迹记录**：停止轨迹记录并安全落盘 GPX（防误触：长按停止）；<br>- **码表模式**：单次骑行里程与极速清零（Trip Reset）；<br>- **诊断页 / 设置页**：快速返回 Page 0 (HOME)。 |
| 双击（DOUBLE） | 预留应用层扩展 |
...
- 7 大配置条目：`FUNCTION MODE`（主功能子视图切换）、`COLOR THEME`（Sunlight 日光 / Dark 暗夜）、`GPS RATE`（10Hz/18Hz/1Hz/5Hz）、`GNSS MODE`、`BRIGHTNESS`、`AUTO SLEEP`、`SENSOR CALIB`。
- 采用更大更粗的 **Oswald 14px** 字体，行高与间距优化，末行底部留有 20px 安全余量，绝不贴底截断。
