...
| 长按（LONG，≥800ms） | - **轨迹记录**：停止轨迹记录并安全落盘 GPX（防误触：长按停止）；<br>- **码表模式**：单次骑行里程与极速清零（Trip Reset）；<br>- **Page 1 (诊断页) / Page 2 (设置页任意状态)**：长按快速返回 Page 0 (HOME)；<br>- **校准流程中**：若未完成校准或需取消校准，长按随时安全退出回退至设置菜单上级。 |
