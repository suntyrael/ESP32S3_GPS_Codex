...
- **数据存储**：GPX 轨迹写入 `/GPX/` 目录，支持温度、G 值等扩展标签。
- **校准数据**：IMU 与磁力计运行时校准，结果保存至 ESP32 NVS，重启自动加载。
...
### 设置菜单（MODE_SETTINGS）
- 菜单项示例：IMU 校准、磁力计校准、GNSS 刷新率（1/5/10/25 Hz）、星座选择、显示亮度、关于/版本。
- 校准流程在后台 FreeRTOS 任务中运行，UI 提示等待进度。
